...
async def send_broadcast(req: BroadcastRequest, user_payload: dict = Depends(get_current_user)):
    ...
    # 2. Calculate Credit Cost (1 Credit per 20 SMS units)
    # Total units = recipients * ceil(message_len / 160)
    # For now, let's simplify to 1 Credit per 20 recipients
    import math
    ...
    supabase.table(\"users\").update({\"credit_balance\": new_balance}).eq(\"clerk_id\", clerk_id).execute()
